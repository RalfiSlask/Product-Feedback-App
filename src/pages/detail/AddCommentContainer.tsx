...
const AddCommentContainer: React.FC<{ errorText: string }> = ({
  errorText
}) => {
  ...
  return (
    <div className='bg-white rounded-[10px] p-6 flex flex-col gap-6'>
      <div className='flex items-center justify-between'>
        ...
        {isError && (
          <p className='text-[#D73737] text-[0.8125rem] font-normal'>
            {errorText}
          </p>
        )}
      </div>
      <textarea
        ...
        className={`${
          isError
            ? 'border-[#D73737]'
            : 'border-[#F7F8FD] hover:border-[#4661E6]'
        } rounded-[5px] bg-[#F7F8FD] p-4 outline-none cursor-pointer w-full h-20 border`}
      ></textarea>
      <div className='flex items-center justify-between'>
        <p className='text-[0.825rem] md:text-[0.9375rem] font-normal'>
          {charactersLeft}
