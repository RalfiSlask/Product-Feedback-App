...
const statusSectionsArray = [
  ...
  },
  {
    id: 2,
    status: 'In-Progress',
    info: 'Features currently being developed',
    color: '#AD1FEA'
  },
...
const MainRoadmapPage = () => {
  ...
  const currIndex = statusSectionsArray.findIndex(
    (element) => element.status === selectedStatus
  );
  ...
  return (
    <div className='flex flex-col items-center w-full'>
      ...
      <main className='bg-[#F7F8FD] w-full md:w-[689px] xl:w-[1110px] flex flex-col items-center mb-[96px]'>
        {windowSize === 'mobile' && (
          <MobileStatusSelector selectionArray={statusSectionsArray} />
        )}
        {windowSize === 'mobile' ? (
          <StatusColumn sectionInfo={statusSectionsArray[currIndex]} />
        ) : (
          <div className='flex justify-between w-full'>
            {statusSectionsArray.map((section) => (
              <StatusColumn key={section.id} sectionInfo={section} />
            ))}
          </div>
        )}
      </main>
