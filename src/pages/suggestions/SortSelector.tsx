...
const SortSelector = () => {
  ...
  return (
    <div
      onClick={handleClick}
      className='flex items-center gap-2 cursor-pointer stroke-white hover:opacity-75 w-44'
    >
      <p className='text-[0.813rem] md:text-[0.875rem] font-normal'>
        Sort by: <span className='font-bold'>{selectedOption}</span>
      </p>
      <div>{arrowUp ? <ArrowUp /> : <ArrowDown />}</div>
    </div>
  );
};
