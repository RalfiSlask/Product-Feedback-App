...
type FeedbackContextVal = {
  ...
  setSelectedFeedback: React.Dispatch<
    React.SetStateAction<ProductRequestsType>
  >;
...
const FeedbackContext = createContext<FeedbackContextVal | undefined>(
  undefined
);
...
export const FeedbackContextProvider: React.FC<ContextType> = ({
  children
}) => {
  const [feedbackList, setFeedbackList] = useState<ProductRequestsType[]>(
    storedFeedbackList ? JSON.parse(storedFeedbackList) : data.productRequests
  );
  const [isAddFeedbackBtnPressed, setIsAddFeedbackBtnPressed] = useState(false);
  const [suggestions, setSuggestions] = useState<ProductRequestsType[]>(
    feedbackList.filter((object) => object.status === 'suggestion')
  );
  const [selectedFeedback, setSelectedFeedback] = useState<ProductRequestsType>(
    storedFeedback ? JSON.parse(storedFeedback) : feedbackList[0]
  );
  ...
  const [upvotedList, setUpvotedList] = useState<number[]>(
    storedUpvoteList ? JSON.parse(storedUpvoteList) : []
  );
    ...
    const updatedList = feedbackList.map((feedback) => {
      if (feedback.id === selectedFeedback.id) {
        return {
          ...
          category: getInputByTypeFromList(
            editInputList,
            'category'
          ).toLowerCase(),
  ...
  const filterSuggestionsByCategory = (text: string) => {
    const suggestionList = feedbackList.filter(
      (object) => object.status === 'suggestion'
    );
    if (text === 'All') {
      ...
      setSuggestions(
        suggestionList.filter(
          (object) => object.category.toLowerCase() === text.toLowerCase()
        )
      );
    }
  };
  ...
  const createNewFeedbackAndAddToList = () => {
    ...
    const categoryInput = getInputByLabel(
      'category',
      newInputList
    ).toLowerCase();
  ...
  useEffect(() => {
    if (selectedFeedback !== null) {
      localStorage.setItem(
        'selectedFeedback',
        JSON.stringify(selectedFeedback)
      );
    }
  }, [selectedFeedback]);
